...
for i in range(args.num_runs):
    ...
    for epoch in range(args.num_epochs):
        ...
        model_all.train()
        model_all[0].set_neighbor_finder(train_ngh_finder)
        ...
        for k in range(0, num_batch):
            ...
            batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = model_all[0].compute_edge_probabilities(
                    source_nodes=sources_batch, destination_nodes=negatives_batch,
                    edge_times=timestamps_batch, edge_idxs=None,
                    n_neighbors=args.num_neighbors,n_filters = args.num_filters)

            batch_src_node_embeddings, batch_dst_node_embeddings = model_all[0].compute_edge_probabilities(
                    source_nodes=sources_batch, destination_nodes=destinations_batch,
                    edge_times=timestamps_batch, edge_idxs=edge_idxs_batch, n_neighbors=args.num_neighbors,n_filters = args.num_filters)

            positive_probabilities = model_all[1](input_1=batch_src_node_embeddings,
                                                      input_2=batch_dst_node_embeddings).squeeze(dim=-1).sigmoid()
            negative_probabilities = model_all[1](input_1=batch_neg_src_node_embeddings,
        ...
        model_all[0].set_neighbor_finder(full_ngh_finder)

        val_losses, val_metrics = eval_edge_prediction(model=model_all, negative_edge_sampler=val_rand_sampler,
                                                       data=val_data, n_neighbors=args.num_neighbors, n_filters = args.num_filters,loss_func=criterion)

        new_node_val_losses, new_node_val_metrics = eval_edge_prediction(model=model_all,
        ...
        early_stop = early_stopping.step(val_metric_indicator, model_all)
